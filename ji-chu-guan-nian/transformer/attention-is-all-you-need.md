...
### **3.3 逐點前饋網絡（Position-wise Feed-Forward Networks）**

* 編碼器和解碼器中的每一層都包含一個全連接前饋網絡，該網絡分别且相同地應用於每個位置 \[15]。
* 該網絡由兩個線性變換組成，中間有一個 ReLU 激活函數 \[15]。

**3.4 嵌入和 Softmax (Embeddings and Softmax)**

* 與其他序列轉導模型類似，Transformer 使用學習嵌入將輸入標記和輸出標記轉換為 d 模型維度的向量 \[16]。
* Transformer 還使用學習的線性變換和 softmax 函數將解碼器輸出轉換為預測的下一個標記概率 \[16]。

**3.5 位置編碼（Positional Encoding）**

* 為了讓模型利用序列的順序信息，向編碼器和解碼器堆棧底部的輸入嵌入添加了「位置編碼」 \[17]。
* 本文使用不同頻率的正弦和餘弦函數作為位置編碼 \[18]。
* 選擇此函數是因為它可以讓模型輕鬆學習通過相對位置進行關注 \[19]。

#### 4. 為什麼選擇自注意力機制？（Why Self-Attention）

* 將自注意力層與通常用於將一個變長符號表示序列映射到另一個等長序列的循環層和卷積層進行比較 \[20]。
* 自注意力層以恆定數量的順序執行操作連接所有位置，而循環層需要 O(n) 個順序操作 \[21, 22]。
* 自注意力層比循環層更快，當序列長度 n 小於表示維度 d 時，這在機器翻譯中最先進的模型使用的句子表示中經常出現 \[22]。
* 與 RNN 序列到序列模型相比，即使僅在 40K 個句子的 WSJ 訓練集上進行訓練，Transformer 也優於 BerkeleyParser \[23]。
...
* 使用標準的 WMT 2014 英德數據集（包含約 450 萬個句子對）進行訓練 \[24]。
* 句子使用字节對編碼進行編碼，該編碼具有大約 37000 個標記的共享源-目標詞彙表 \[24]。
* 句子對按近似序列長度批次處理在一起 \[24]。

**5.2 硬件和時間安排 (Hardware and Schedule)**

* 在一台配備 8 個 NVIDIA P100 GPU 的機器上訓練模型 \[25]。

**5.3 優化器 (Optimizer)**

* 使用 Adam 優化器，並在訓練過程中改變學習率 \[25, 26]。

**5.4 正則化 (Regularization)**

* 在訓練期間採用了三種類型的正則化：
* 殘差丟失：將丟失應用於每個子層的輸出 \[27]。
* 標籤平滑：在訓練期間，使用值 εls=0.1 的標籤平滑 \[28]。

#### 6. 結果 (Results)

**6.1 機器翻譯 (Machine Translation)**

* 在 WMT 2014 英德翻譯任務中，大型 Transformer 模型（表 2 中的 Transformer（大））的性能優於之前報告的最佳模型（包括集成模型）超過 2.0 BLEU，創造了 28.4 的全新 BLEU 分數 \[29]。
* 在 WMT 2014 英法翻譯任務中，大型模型的 BLEU 得分為 41.0，優於之前發布的所有單一模型，而訓練成本不到之前最先進模型的 1/4 \[30]。

**6.2 模型變體 (Model Variations)**

* 為了評估 Transformer 不同組件的重要性，論文改變了基本模型的不同之處，測量了英德翻譯在開發集 newstest2013 上的性能變化 \[31]。
* 結果顯示在表 3 中。

**6.3 英語成分句法分析 (English Constituency Parsing)**

* 為了評估 Transformer 是否可以泛化到其他任務，論文對英語成分句法分析進行了實驗 \[32]。
* 結果顯示在表 4 中，表明儘管缺乏針對特定任務的調整，但模型的性能出奇地好，產生的結果優於之前報告的所有模型，除了循環神經網絡語法 \[33]。

#### 7. 結論 (Conclusion)

* 本文介紹了 Transformer，這是第一個完全基於注意力的序列轉導模型，它用多頭自注意力機制取代了編碼器-解碼器架構中最常用的循環層 \[23]。
* 對於翻譯任務，Transformer 的訓練速度明顯快於基於循環層或卷積層的架構 \[23]。
* 在 WMT 2014 英德和英法翻譯任務中，Transformer 都實現了新的最先進水平 \[23]。

#### 總結

**Transformer 是一種新穎的序列轉導模型，它完全依賴注意力機制，避免了傳統循環神經網絡的缺點。** 它在機器翻譯等任務上取得了最先進的結果，並展現出在其他自然語言處理任務中的巨大潛力。 Transformer 的出現標誌著自然語言處理領域的一個重要里程碑，並為未來的研究開闢了新的方向。
